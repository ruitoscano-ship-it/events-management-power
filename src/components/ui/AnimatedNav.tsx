...
export function AnimatedNav<T extends string>({
  ...
  return (
    <nav className={`border-b border-[#2a2a3d] bg-[#0a0a12] ${className}`}>
      <div
        ref={rowRef}
        className="nav-scroll relative mx-auto flex max-w-7xl gap-2 px-4 sm:gap-4 sm:px-4 md:gap-6"
      >
        <span
        ...
        {tabs.map((t) => (
          <button
            ...
            className={`relative z-10 shrink-0 min-h-12 px-3 py-3 text-xs font-bold tracking-wider transition-colors duration-200 sm:min-h-0 sm:px-2 sm:py-3 sm:tracking-widest sm:text-xs ${
              active === t.id
                ? 'text-white'
